...
```{r, Actual Archaeological Site Locations, Point Conception}
...
# Fitting logistic regression model, using `water_proximity` directly
logit_mod_water <- glm(presence ~ elevation_m + water_proximity, data = all_sites, family = binomial)

# Fitting logistic regression model, using transformed `water_proximity`
logit_mod_transform <- glm(presence ~ elevation_m + sqrt(water_proximity), 
                 data = all_sites, 
                 family = binomial)

# Extracting the residuals
all_sites$residual <- resid(logit_mod_transform)
...
logit_mod_transform_spatial <- glm(presence ~ elevation_m + sqrt(water_proximity) + lag_elevation, 
                         data = all_sites, 
                         family = binomial)

# Adding the predicted probability to all_sites
all_sites$predicted_prob <- predict(logit_mod_transform_spatial, type = "response")

# Model summary
summary(logit_mod_water)
summary(logit_mod_transform_spatial)
...
logit_transform_hypo <- glm(presence ~ elevation_m + sqrt(water_proximity), 
                 data = hypo_sites, 
                 family = binomial)

hypo_sites$residual <- resid(logit_transform_hypo)
...
logit_transform_hypo_spatial <- glm(presence ~ elevation_m + sqrt(water_proximity) + lag_elevation, 
                         data = hypo_sites, 
                         family = binomial)

summary(logit_transform_hypo_spatial)

hypo_sites$predicted_prob <- predict(logit_transform_hypo_spatial, type = "response")
```
